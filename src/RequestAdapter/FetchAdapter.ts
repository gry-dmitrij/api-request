...
export default class FetchAdapter extends AbstractRequestAdapter {
  ...
  private async _getDataFromResponse(response: Response, config?: TRequestConfig): Promise<unknown> {
  ...
  private async _fetch<T = unknown>(request: Request, config?: TRequestConfig): Promise<ApiResponse<T>> {
  ...
  request<T = unknown>(
    ...
    return this._fetch<T>(request, config)
  }
}
